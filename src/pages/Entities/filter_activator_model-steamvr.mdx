...
import filter_activator_modelIcon from '@site/static/img/Entities/steamvr/filter_activator_model.png';

<img src={filter_activator_modelIcon} alt="filter_activator_model icon" style={{height: '80px'}} />
...
- **Filter Model** (`model`) \<`Studio`\>\
The model to filter by. If the filter mode is Allow, only entities whose model matches the given string will pass the filter. If the filter mode is Disallow, all entities EXCEPT those whose model matches the string will pass the filter.
