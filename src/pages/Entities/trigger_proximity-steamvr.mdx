...
import trigger_proximityIcon from '@site/static/img/Entities/steamvr/trigger_proximity.png';

<img src={trigger_proximityIcon} alt="trigger_proximity icon" style={{height: '80px'}} />
...
- **Point to Measure From** (`measuretarget`) \<`TargetDestination`\>\
The name of a target entity who's origin is the point to measure the player's distance from.

- **Radius to measure within** (`radius`) \<`String`\>\
