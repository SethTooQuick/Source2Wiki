...
import snd_event_path_cornerIcon from '@site/static/img/Entities/cs2/snd_event_path_corner.png';

<img src={snd_event_path_cornerIcon} alt="snd_event_path_corner icon" style={{height: '80px'}} />
...
- **Sound Event Name** (`soundName`) \<`Sound`\>\
Name of the sound event to play

- **Start On Spawn** (`startOnSpawn`) \<`Boolean`\>\
Play the sound immediately upon spawning

- **Path Corner Entity Name** (`pathCornerEntity`) \<`TargetDestination`\>\
Name of the first path corner node in the chain

- **Max Soundevents** (`soundeventCountMax`) \<`Integer`\>\
The max number of soundevents the path corner chain will emit

- **Max Distance** (`distanceMax`) \<`Float`\>\
The distance beyond which a segment will not emit a soundevent

- **Max Dot Product** (`dotProductMax`) \<`Float`\>\
For excluding soundevents that are close to eachother
