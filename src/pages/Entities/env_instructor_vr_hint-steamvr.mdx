...
import env_instructor_vr_hintIcon from '@site/static/img/Entities/steamvr/env_instructor_vr_hint.png';

<img src={env_instructor_vr_hintIcon} alt="env_instructor_vr_hint icon" style={{height: '80px'}} />
...
- **Start Sound** (`hint_start_sound`) \<`String`\>\
...
- **Layout Type** (`hint_layoutfiletype`) \<`Choices`\>\
Panorama Layout file to be used.  Generic Hand, World or Custom.  Custom requires specifying .xml file
  - Default 
  - Custom 


- **Custom Layout File** (`hint_custom_layoutfile`) \<`String`\>\
Custom Path to a xml file ie - file://\{resources\}/layout/custom_hint.xml

- **Panel Attach Type** (`hint_vr_panel_type`) \<`Choices`\>\
Attach Type for the Panorama Panel
  - None 
  - Dominant Hand 
  - Off Hand 
  - Entity 
...
- **Height Offset** (`hint_vr_height_offset`) \<`Float`\>\
