...
- **Test Mode** (`testMode`) \<`Choices`\>
  - None 
  - Oscillate Pose Parameters 
  - Add/Remove Animation Layers 
  - Report Animation Events 
  - Toggle Base Layer Sequence 
  - Test Model Scale 
  - Client-Side Ragdoll 
  - Use Procedural Bones 
  - Reset Sequences 


- **Use Client-Side Animation** (`doClientSideAnimation`) \<`Boolean`\>

- **Sequence Name** (`sequenceName`) \<`Sequence`\>\
Default animation sequence for the model to be playing after spawning.

- **Sequence Name 2** (`sequenceName2`) \<`Sequence`\>\
Used by the 'Toggle Base Layer Sequence' test.

- **Pose Parameter Name** (`poseParameterName`) \<`String`\>\
Pose parameter to use for various unit tests

- **Layer 1 Sequence Name** (`layerSequence1`) \<`Sequence`\>\
Layer sequence #1 for the Add/Remove Animation Layers test.

- **Layer 2 Sequence Name** (`layerSequence2`) \<`Sequence`\>\
Layer sequence #2 for the Add/Remove Animation Layers test.
...
- AddSequenceLayer \<`Void`\>\
Adds a random sequence as a new layer

- ClearAllSequenceLayers \<`Void`\>\
Clears all sequence layers
