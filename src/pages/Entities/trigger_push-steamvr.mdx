...
import trigger_pushIcon from '@site/static/img/Entities/steamvr/trigger_push.png';

<img src={trigger_pushIcon} alt="trigger_push icon" style={{height: '80px'}} />
...
- **Push direction is expressed in local space** (`pushdir_islocal`) \<`Boolean`\>\
If checked, the push direction angle is in local space and will be rotated by the entity's angles before being applied.
...
- **Trigger on Start Touch** (`triggeronstarttouch`) \<`Boolean`\>\
...
- SetPushSpeed \<`Integer`\>\
Set the push speed in inches / second.

- SetPushDirection \<`Vector`\>\
Sets the angles indicating the direction to push touched entities.
