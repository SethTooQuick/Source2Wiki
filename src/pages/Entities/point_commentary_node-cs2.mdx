...
- **Prevent Movement?** (`prevent_movement`) \<`Choices`\>\
If set, the player will be unable to move while this node is playing.
  - No 
  - Yes 


- **Start Disabled?** (`start_disabled`) \<`Choices`\>\
If set, this node will be disabled on startup, and will require the Enable input to be fired before it will be visible to the player.
  - No 
  - Yes 
...
- StartCommentary \<`String`\>\
Forces a node to start playing.

- StartUnstoppableCommentary \<`String`\>\
...
- SetNewOrigin \<`TargetDestination`\>\
...
- OnCommentaryStarted \<`Void`\>\
Fired when this node starts playback.

- OnCommentaryStopped \<`Void`\>\
