...
- **Particle Snapshot File** (`snapshot_file`) \<`String`\>\
Name of .PSF (Particle Snapshot File) to be loaded and used by this particle system (e.g. 'smoke_snapshot1')

- **Particle Snapshot Mesh** (`snapshot_mesh`) \<`NodeId`\>\
Id of a mesh in the map to be used to generate a particle snapshot, if specified Particle Snapshot File is ignored.
