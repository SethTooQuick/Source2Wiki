...
import snd_opvar_set_aabbIcon from '@site/static/img/Entities/steamvr/snd_opvar_set_aabb.png';

<img src={snd_opvar_set_aabbIcon} alt="snd_opvar_set_aabb icon" style={{height: '80px'}} />
...
- **Box Direction** (`AABBDirection`) \<`Choices`\>\
Limit the evaluation of the box to a single axis
  - Any 
  - X Only 
  - Y Only 
  - Z Only 
...
- **Box Inner Mins** (`box_inner_mins`) \<`Vector`\>

- **Box Inner Maxs** (`box_inner_maxs`) \<`Vector`\>
