...
import env_shooterIcon from '@site/static/img/Entities/cs2/env_shooter.png';

<img src={env_shooterIcon} alt="env_shooter icon" style={{height: '80px'}} />
...
- **Gib Direction (Pitch Yaw Roll)** (`angles`) \<`String`\>\
...
Select an entity to specify a location to sample lighting from for all gibs spawned by this shooter, instead of their own origins.
...
Thing to shoot out. Can be a .vmdl or a .vmat.
...
- **Spawnflags** (`spawnflags`) \<`Flags`\>
  - On fire 
  - strict remove after lifetime 


- **Disable Shadows on Gibs** (`nogibshadows`) \<`Boolean`\>
