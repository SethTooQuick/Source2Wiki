...
import env_volumetric_fog_volumeIcon from '@site/static/img/Entities/steamvr/env_volumetric_fog_volume.png';

<img src={env_volumetric_fog_volumeIcon} alt="env_volumetric_fog_volume icon" style={{height: '80px'}} />

Point Entity

Sets a user bounding volume for volumetric fog - if one of these are in the map then all fog will get clamped to it. More than one then fog gets rendered if it's inside any of them.
...
- **Falloff Exponent** (`FalloffExponent`) \<`Float`\>\
Controls the fall off curve for the fog at the edges

- **Falloff Shape** (`Shape`) \<`Choices`\>
  - Linear (Box) Falloff 
  - Radial (Sphere) Falloff 
