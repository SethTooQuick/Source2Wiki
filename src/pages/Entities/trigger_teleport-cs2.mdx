...
import trigger_teleportIcon from '@site/static/img/Entities/cs2/trigger_teleport.png';

<img src={trigger_teleportIcon} alt="trigger_teleport icon" style={{height: '80px'}} />
...
- **Remote Destination** (`target`) \<`TargetDestination`\>\
The entity specifying the point to which entities should be teleported.

- **Local Destination Landmark** (`landmark`) \<`TargetDestination`\>\
If specified, then teleported entities are offset from the target by their initial offset from the landmark.

- **Use Landmark Orientations** (`use_landmark_angles`) \<`Boolean`\>\
Rotate around landmark by relative landmark orientations

- **Mirror Player** (`mirror_player`) \<`Boolean`\>\
Mirror the player teleport position through the plane defined by the trigger's origin and up vector

- **Only Spawn-Valid Destinations** (`check_if_dest_clear_for_player`) \<`Boolean`\>\
Only teleport if there is a clear teleport destination.
