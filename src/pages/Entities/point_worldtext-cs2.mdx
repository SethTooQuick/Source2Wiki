...
- **Message** (`message`) \<`TextBlock`\>

- **Start Enabled** (`enabled`) \<`Boolean`\>

- **Draw Fullbright** (`fullbright`) \<`Boolean`\>

- **Color** (`color`) \<`Color255Alpha`\>

- **World Units Per Pixel** (`world_units_per_pixel`) \<`Float`\>

- **Font Size** (`font_size`) \<`Float`\>

- **Font Name** (`font_name`) \<`String`\>

- **Horizontal Justify** (`justify_horizontal`) \<`Choices`\>
  - Left 
  - Center 
  - Right 


- **Vertical Justify** (`justify_vertical`) \<`Choices`\>
  - Bottom 
  - Center 
  - Top 


- **Reorient Mode** (`reorient_mode`) \<`Choices`\>
  - None 
  - Rotate Around Up 


- **Depth Offset** (`depth_render_offset`) \<`Float`\>
