...
import npc_template_makerIcon from '@site/static/img/Entities/steamvr/npc_template_maker.png';

<img src={npc_template_makerIcon} alt="npc_template_maker icon" style={{height: '80px'}} />
...
- **Name of Destination Group** (`DestinationGroup`) \<`TargetDestination`\>\
...
- **Dest. Criterion: Visible to player?** (`CriterionVisibility`) \<`Choices`\>\
...
- **Dest. Criterion: Distance to player?** (`CriterionDistance`) \<`Choices`\>\
...
- **Frequency** (`SpawnFrequency`) \<`Integer`\>\
How often (in seconds) a new NPC will be spawned. If set to -1, a new NPC will be made when the last NPC dies.

- **Retry Frequency** (`RetryFrequency`) \<`Integer`\>\
How long (in seconds) to wait before trying again after failing to spawn an NPC when no valid spot could be found. If set to -1, Spawn Frequency is used.
...
- **Hull Check Mode** (`HullCheckMode`) \<`Choices`\>\
...
- **Spawnflags** (`spawnflags`) \<`Flags`\>
  - Don't preload template models 


- **Name of template NPC** (`TemplateName`) \<`TargetDestination`\>\
